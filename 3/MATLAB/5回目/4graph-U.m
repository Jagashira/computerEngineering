...
time=linspace(1,TIME,TIME); %描画のためUU　高さの配列の書記か
U=linspace(1,TIME,TIME);
%*************************************%
%******計算しているのはここから********%
%*************************************%
for i=1:tf
  ...
  %水位の計算
  x = x +(U(i) + V())/S;     %V() は外乱 -1< V() < 1 のランダムな値 最初は無視
...
plot(time,U,'*','Color',[0.4940 0.1840 0.5560]);%x(t)の計算結果
hold on;
title('収束結果');
xlabel('time');
ylabel('Hight x(t)');
